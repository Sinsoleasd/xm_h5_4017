...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      //type：为了区分这三个元素
      //diaNum形参：变化量（1）变化量（-1）input最终的个数（并不是变化量
      //cart：哪一个产品【身上有id】
      //向服务器发请求，修改数量
      switch (type) {
        //加号
        ...
        case "minus":
          ...
          break;
        case "change":
          //用户输入进来的最终量，如果非法的（带汉字|出现负数），  带给服务器数字
    ...
    async deleteCartById(cart) {
      try {
        ...
        await this.$store.dispatch("getDeleteCartById", cart.skuId);
        this.getData();
      } catch (error) {
        alert(error, message);
      }
  ...
  computed: {
    ...
    //判断底部复选框是否勾选【全部的产品都选中，才勾选】
    isAllCheck() {
      //遍历数组里面原理：只要全部元素isAllCheck属性都为1====》真true
      //只要有一个不是1===》假false
