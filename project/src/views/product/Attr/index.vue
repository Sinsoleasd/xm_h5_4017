      ...
      <div v-show="isShowTable">
        ...
        <el-table style="width: 100%" border :data="attrList">
          ...
          <template slot-scope="{ row, $index }">
            <el-tag
              type="success"
              v-for="(attrVaule, index) in row.attrVauleList"
              :key="attrVaule.id"
              style="margin: 0px 10px"
              >{{ attrVaule.vauleName }}</el-tag
            >
          </template>
          <el-table-column
            prop="prop"
      ...
      <div v-show="!isShowTable">
        ...
        >
          ...
          <el-table-column label="操作" prop="prop" width="width">
            <template slot-scope="{ row, $index }">
              <!-- 气泡确认框 -->
              <el-popconfirm :title="`确定删除？${row.vauleName}`" @onConfirm="deletAttrValue($index)">
                <el-button
                  type="danger"
                  icon="el-icon-delete"
                  size="mini"
  ...
  methods: {
    ...
    toLook(row) {
      ...
      let isRepat = this.attrInfo.attrValueList.some((item) => {
        ...
        if (row !== item) {
          return row.vauleName == item.vauleName;
        }
      });
      if (isRepat) return;
