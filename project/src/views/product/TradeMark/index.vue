<template>
  <div>
    ...
    layout：可以实现分页器布局
    pager-count:按钮的数量 如果是9 连续的页码是7
  -->
    <el-pagination
    ...
    >
      <!-- form表单 v-model属性：这个属性的作用是，把表单的数据数据的哪个对象的身上，将来表单验证，也需要这个属性-->
      <el-form
        style="width: 80%"
        :model="tmForm"
        :rules="rules"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        //品牌名称的验证规则
        //required:必须要校验字段（前面的五角星有关系） message:提示信息 trigger:用户行为设置（事件的设置：blur，change）
        tmName: [
  ...
  methods: {
    ...
    addOrUpdataTradeMark() {
      ...
      this.$refs.ruleForm.validate(async (success) => {
        ...
        if (success) {
          ...
          //发请求（添加品牌|修改品牌）
          let resutl = await this.$API.trademark.reqAddOrUpdateTradeMark(
            this.tmForm
          );
          if (resutl.code == 200) {
            //弹出信息：添加品牌成功，修改品牌成功
    ...
    //删除品牌的操作
    deleteTradeMark(row){
      this.$confirm(`你确定删除${row.tmForm}?`, '提示', {
